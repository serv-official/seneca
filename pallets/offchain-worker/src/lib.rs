...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		fn send_request(
			...
			let pending = request.deadline(deadline).send().map_err(|_| HttpError::Invalid)?;

			let response = pending.try_wait(deadline).map_err(|_| HttpError::IoError)?;
			...
			let json_response = match Self::parse_json(response_str) {
				Ok(json) => json,
				Err(_) => return Err(HttpError::Invalid),
			};
