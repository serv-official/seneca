...
fn it_works_for_create_schema() {
	new_test_ext().execute_with(|| {
		...
		let mandatory_fields =
			Attribute { name: b"name".to_vec(), attribute_type: AttributeType::Hex };
		let issuance_req =
			IssuanceRequirement { name: b"insuance".to_vec(), insuance_type: IssuanceType::Int };
		let claim = Claim {
...
fn it_works_for_create_credential() {
	new_test_ext().execute_with(|| {
		...
		let credential_holder =
			b"did:seneca:5GFEtniprMeFuh8HcoVrWxz4aQtv6T5V9bkENSnfPYhY4p8H".to_vec();
		let nonce = 2u64;
		// Encode and sign the schema message.
		...
		// Dispatch a signed create schema extrinsic.
		assert_ok!(SchemaRegistry::create_credential(
			RawOrigin::Signed(signer).into(),
			schema_id,
			context.clone(),
			schema,
...
fn it_works_for_update_credential() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SchemaRegistry::update_credential(
			RawOrigin::Signed(signer).into(),
			schema_id.clone(),
			(updated_sig.clone(), credential.clone())
		));
		assert_eq!(
			SchemaRegistry::credential_registry(schema_id.clone()),
			Some((updated_sig, credential))
		);
	})
}
...
fn it_works_for_delete_credential() {
	new_test_ext().execute_with(|| {
		...
		// Dispatch a signed extrinsic.
		assert_ok!(SchemaRegistry::delete_credential(
			RawOrigin::Signed(signer).into(),
			schema_id.clone()
		));
		// Read pallet storage and assert an expected result.
