...
use types::VerifiableCredenialSchema;
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10_000 + T::DbWeight::get().reads_writes(1,1).ref_time())]
		pub async fn create_public_did(
			client: &Client<impl System>,
			vc_credential: VerifiableCredenialSchema,
		) -> Result<(), Box<dyn std::error::Error>> {
			let call = Call::new(vc_credential.id, vc_credential);
			let extrinsic = client.create_signed_extrinsic(call, None)?;
			let hash = client.submit_extrinsic(extrinsic).await?;
			println!("Data record created with ID {}", hash);
			Ok(())
		}
		#[pallet::weight(10_000 + T::DbWeight::get().reads_writes(1,1).ref_time())]
		pub async fn update_public_did(
			client: &Client<impl System>,
			id: u64,
			schema: String,
			values: String,
		) -> Result<(), Box<dyn std::error::Error>> {
			let vc_credential = VerifiableCredenialSchema { id, schema, values };
			let call = Call::new(id, vc_credential);
			let extrinsic = client.create_signed_extrinsic(call, None)?;
			let hash = client.submit_extrinsic(extrinsic).await?;
			println!("Data record with ID {} updated", hash);
			Ok(())
		}
		#[pallet::weight(10_000 + T::DbWeight::get().reads_writes(1,1).ref_time())]
		pub async fn delete_public_did(
			client: &Client<impl System>,
			id: u64,
		) -> Result<(), Box<dyn std::error::Error>> {
